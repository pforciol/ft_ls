...
/*   Updated: 2019/06/17 13:15:46 by pforciol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/ft_ls.h"

static t_data		*ls_getdata(t_data *data, char *path, struct stat *p_stat)
{
	...
	data->uid = *(getpwuid(p_stat->st_uid));
	data->gid = *(getgrgid(p_stat->st_gid));
...
t_list				*ls_getarg(char *arg, t_list *l_args, t_opt *opt, int *v_a)
{
	...
	if (lstat(arg, &p_stat) != 0)
		ls_perror(arg, 0);
	else if ((data = ls_getdata(data, arg, &p_stat)) != NULL)
	{
